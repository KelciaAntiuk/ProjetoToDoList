...
const getAll = async () => {
  const [cadastro] = await connection.execute('SELECT * FROM users');
  return cadastro;

};

const createCadastro = async (cadastro) => { //Recebemos um cadastro e vamos 'pegar ela
...
const deleteCadastro = async (id) => {
  ...
  const code = 'DELETE FROM users WHERE id = ? '
...
const updateCadastro = async (id, cadastro) => {
  ...
  const code = 'UPDATE users SET title = ? WHERE id = ? '
